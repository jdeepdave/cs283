...
int get_student(int fd, int id, student_t *s) {
    if (id < 1 || id > 100000) {
        return SRCH_NOT_FOUND;  // ID out of range
    }

    lseek(fd, 0, SEEK_SET);  // Start at the beginning of the file

    while (read(fd, s, sizeof(student_t)) == sizeof(student_t)) {
        if (s->id == id) {
            return NO_ERROR;  // Found the student
        }
    }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa) {
    ...
    student_t new_student = {0};  // Initialize empty student
    new_student.id = id;
    ...
    // Extend the file if needed before writing
    ...
    // Ensure the file expands properly
    int new_file_size = (id + 1) * sizeof(student_t);
...
int compress_db(int fd) {
    int new_fd = open("temp.db", O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (new_fd == -1) {
        return ERR_DB_FILE;
    }
    ...
    while (read(fd, &student, sizeof(student_t)) == sizeof(student_t)) {
        if (student.id != 0) {
            write(new_fd, &student, sizeof(student_t));
        }
    }
